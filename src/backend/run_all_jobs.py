...
def run_all_jobs(
    ...
    # Dump CSV at the end of all jobs #ML add
    dump_nested_dict_to_csv(formatted_results, out_subdirs[0])
...
def extract_scalar(subdict: dict, task_key: str):
    ...
    if "measurement" in subdict:
        meas = subdict["measurement"]
        if isinstance(meas, dict):
            # if dict contains a scalar directly
            if "value" in meas:
                return meas["value"]
            # or just grab the first numeric-looking entry
            for key, value in meas.items():
                if isinstance(value, (int, float)):
                    return value
                if isinstance(value, dict) and "measured" in value:
...
def format_results(results: dict) -> dict:
    """Formats the raw results from running all Hazen jobs into nested dict structure.
    Preserves metadata but always adds a scalar 'value' field for reporting.

    # This has been altered a lot to extract scalars etc from the nested dict , ML
    """
    formatted_results = nested_dict()

    for subdict in [r for r in results if r is not None]:
        # file string (handles both str and list cases)
        file_value = subdict["file"]
        file_str = file_value[0] if isinstance(file_value, list) else file_value

        # task name mapping
        task_key = CLASS_STR_TO_TASK.get(subdict["task"], subdict["task"])

        # best matching coil / orientation with fallbacks
        coil_key = substring_matcher(file_str, EXPECTED_COILS) or "UnknownCoil"
        orientation_key = substring_matcher(file_str, EXPECTED_ORIENTATIONS) or "UnknownOrientation"

        # add scalar value alongside metadata
...
#     Adds debug prints and safe fallbacks for keys.
...
#         # get task name by mapping task class back to string, fallback to raw name
#         task_key = CLASS_STR_TO_TASK.get(subdict["task"], subdict["task"])

#         # get best matching coil by matching file description with expected coils, fallback to 'UnknownCoil'
#         coil_key = substring_matcher(file_str, EXPECTED_COILS) or "UnknownCoil"

#         # get best matching orientation by matching file description with expected orientations, fallback to 'UnknownOrientation'
#         orientation_key = substring_matcher(file_str, EXPECTED_ORIENTATIONS) or "UnknownOrientation"

#         # DEBUG PRINTS: show exactly what keys and values are being processed
#         print("TASK:", subdict["task"], "->", task_key)
#         print("COIL KEY:", coil_key)
#         print("ORIENTATION KEY:", orientation_key)
#         # Optional: print value if you have extract_scalar function
#         # print("VALUE:", extract_scalar(subdict))
