...
class FrameConfig(tk.Frame):
    ...
    def _read_config_settings(self):
        ...
        get_queue().put(("SWITCH_FRAME", "FRAME_TASKRUNNER", args_to_pass))

    def handle_event(self, event):
        if event[0] in (
            "UPDATE: PROGRESS BAR DICOM CHECKING",
            "UPDATE: PROGRESS BAR DICOM SORTING",
        ):
            self.modal_progress.add_progress(event[1])

        elif event == "FINISH: PROGRESS BAR DICOM CHECKING":
            self.modal_progress.destroy()
            self.modal_progress = ProgressBarModal(self, "Sorting loose DICOMs")

        elif event == "FINISH: PROGRESS BAR DICOM SORTING":
            self.modal_progress.destroy()
